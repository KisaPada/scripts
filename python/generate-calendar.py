...
def _generate_calendar(  # {
    ...
    with open(file_name, 'w') as file:
        print("---\ntags:\n    - calendar\n---", end='\n\n', file=file)
        print(f"[[{prev_year}-{prev_month:02d}]]", end='\n\n', file=file)
        for date in cal.itermonthdates(year, month):
            if date.month == month:
                print(date.strftime(format_string), end="\n", file=file)
        print(f"\n[[{next_year}-{next_month:02d}]]", end='\n', file=file)
...
if __name__ == "__main__":  # {
    ...
    parser.add_argument("-y", "--year", type=int, help="choose a year in YYYY format")
    ...
    if args.month:
        try:
            current_date_date_list = [datetime.datetime.strptime(f"{args.month}-01", "%Y-%m-%d")]
        except ValueError:
            print("Error: -m argument must be given in YYYY-MM format")
            sys.exit(1)
    else:
        current_date_list = [datetime.datetime.today()]
    ...
    if args.year:
        ...
        for month in range(1, 13):
            try:
                current_date_list.append(datetime.datetime.strptime(f"{args.year}-{month}-01", "%Y-%m-%d"))
            except ValueError:
                print("Error: -y argument must be given in YYYY format")
                sys.exit(1)
    ...
    for current_date in current_date_list:
        ...
        if args.year:
            prev_year, prev_month = _get_target_month(current_date, offset=-1)
            year, month = _get_target_month(current_date, offset=0)
            next_year, next_month = _get_target_month(current_date, offset=1)
            rtn += _generate_calendar(year, month, prev_year, prev_month, next_year, next_month, force, weekday)
        ...
        if args.next:
            prev_year, prev_month = _get_target_month(current_date, offset=0)
            year, month = _get_target_month(current_date, offset=1)
            next_year, next_month = _get_target_month(current_date, offset=2)
            rtn += _generate_calendar(year, month, prev_year, prev_month, next_year, next_month, force, weekday)

        # Generate calendar for previous month
        if args.previous:
            prev_year, prev_month = _get_target_month(current_date, offset=-2)
            year, month = _get_target_month(current_date, offset=-1)
            next_year, next_month = _get_target_month(current_date, offset=-0)
            rtn += _generate_calendar(year, month, prev_year, prev_month, next_year, next_month, force, weekday)
    sys.exit(rtn)
# }
